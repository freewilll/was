...
void assert_relocations(ElfSection* section, ...) {
    ...
    while (1) {
        ...
        if (expected_type != got_type || expected_offset != got_offset || expected_addend != got_addend) {
            dump_relocations(section);
            panic("Mismatch at position %d: expected %#x, %#x, %d, got %#x, %#x, %ld",
